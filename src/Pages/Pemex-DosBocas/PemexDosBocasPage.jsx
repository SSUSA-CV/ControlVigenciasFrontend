...
export const PemexDosBocasPage = () => {
  ...
  useEffect(() => {
    axios.get('https://controlvigenciasbackend-szwc.onrender.com/api/operadoresD', { withCredentials: true })
    .then(response => setData(response.data))
    .catch(error => setError(error.response.data))
  }, [eliminado])

  const handleDelete = (id) => {
    axios.delete(`https://controlvigenciasbackend-szwc.onrender.com/api/operadoresD/${id}`, { withCredentials: true })
    .then(response => {
  ...
  const handleFiltrarProximos = () => {
    axios.get('https://controlvigenciasbackend-szwc.onrender.com/api/operadoresD_proximos', { withCredentials: true })
    .then(response => {
  ...
  const handleFiltrarId_pase = e => {
    ...
    axios.get(`https://controlvigenciasbackend-szwc.onrender.com/api/operadoresD/${id_pase}`, { withCredentials: true })
    .then(response => setData(response.data))
    .catch(error => {
      setData([])
  ...
  const handleFiltrarNombre = e => {
    ...
    axios.get(`https://controlvigenciasbackend-szwc.onrender.com/api/operadoresD_nombre/${buscar}`, { withCredentials: true })
    .then(response => setData(response.data))
    .catch(error => {
      setData([])
  ...
  return (
    <div className='container-main'>
      <h1 className='titulo'>VIGENCIAS PEMEX - Dos Bocas</h1>
      {
        rol && <button onClick={() => navigator('/crear-pemex-dosbocas')} className='añadir'>Añadir registros</button>
      }
                    ...
                    <td>{dato.id_pase}</td>
                    {
                      rol && (
                        <td className='text-align-center'>
                          <button onClick={() => navigator(`/editar-pemex-dosbocas/${dato.id}`)} className='accion'><span className='material-symbols-outlined editar'>edit</span></button>
